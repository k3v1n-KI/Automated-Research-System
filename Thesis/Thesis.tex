...
To address the subsequent maintenance problem, Project 2 introduces Pathways, an interactive service-search and directory-maintenance platform. Pathways uses an event-sourced data model and a Find--Verify--Ask--Close flywheel to capture human validation and support longitudinal directory maintenance. Four focused evaluations characterize the combined system. A 50-query extraction benchmark achieved micro-F1 of 0.917, although exact-case accuracy fell from 1.00 on canonical queries to 0.36 on paraphrases. On an independently adjudicated ranking set, BM25 achieved higher nDCG@5 than the current Pathways ranker (0.916 versus 0.804). A simulated participation sweep showed that day-90 freshness increased from 0.00 with no verification participation to 1.00 under full participation. Finally, a 500-field restoration benchmark produced 499 entity-supported fills, including 142 exact matches, with no wrong-entity fills and one unresolved field. These results support Pathways as an auditable maintenance prototype, while identifying natural-language generalization, ranking calibration, simulation realism, and source-level validation as limitations.
...
\centering
\begin{tikzpicture}[
  scale=0.80, transform shape, % Scaled slightly to fit the wider diamond on the page
  node distance=2cm and 2cm, 
  auto, thick,
  process/.style={rectangle, draw, fill=green!10, text width=3.2cm, text centered, minimum height=1.4cm, rounded corners}, 
  % THE FIX: aspect=2 stretches it horizontally, minimum width makes it much larger
  ...
  % Pushed out slightly to make room for the larger diamond
  ...
  % THE FIX: Using explicit anchors (.south, .north, .west, .east) guarantees arrows stop exactly at the border tips
  ...
  % The long return arrow cleanly wrapping around the left
...
Study 1 evaluates whether ordinary requests can be converted into the structured filters used by Pathways search. The expanded fixture contains 50 queries drawn from the full normalized resource vocabulary: 25 canonical queries using recognized terms and 25 paraphrases using more natural expressions. Gold labels cover need, modality, population, coverage, location, language, and urgency. We report exact-case accuracy, field-level exact accuracy, and micro-averaged precision, recall, and F1.
...
Study 4 evaluates the restoration workflow on a sealed 500-field benchmark. The hybrid condition retains the fuzzy entity-supported Places results and sends unresolved cases through the ARS fallback. We report requested fields, filled fields, entity-supported fields, exact matches, wrong-entity fields, unresolved fields, and unexpected fills.
...
1 & ``mental health help for a teenager in Scarborough'' & Location extracted; population omitted & Paraphrase limitation, not a ranking failure \\
...
\midrule
1 & Natural-language filter extraction & 50 queries; 25 canonical and 25 paraphrased & Micro-F1 $=0.917$; exact-case accuracy $=0.680$ \\
...
4 & Missing-data restoration & 500 requested fields; hybrid Places plus ARS fallback & 499 supported fills; 142 exact; 0 wrong entity \\
\bottomrule
\end{tabularx}
\end{table}

\subsubsection{Study 1 results}
The canonical queries were extracted perfectly in this controlled fixture, but performance declined on paraphrases. Overall micro-precision was 0.984, micro-recall was 0.858, and micro-F1 was 0.917. Exact field accuracy was highest for location (1.00), need (0.96), coverage (0.96), and language (0.98), and lowest for population (0.86) and modality (0.90). The gap between canonical exact-case accuracy (1.00) and paraphrase exact-case accuracy (0.36) shows that the current rule-based vocabulary is reliable when users employ expected terms but does not yet provide broad language understanding.

\begin{table}[H]
\caption{Study 1 filter-extraction results}
\label{tab:study1_results}
\centering
\small
\begin{tabular}{lccc}
\toprule
\textbf{Query stratum} & \textbf{Cases} & \textbf{Exact-case accuracy} & \textbf{Micro-F1} \\
\midrule
Canonical & 25 & 1.000 & -- \\
Paraphrased & 25 & 0.360 & -- \\
All queries & 50 & 0.680 & 0.917 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    ymin=0, ymax=1.05, ylabel={Accuracy},
    symbolic x coords={Canonical,Paraphrased,Overall}, xtick=data,
    nodes near coords, nodes near coords style={font=\small},
    enlarge x limits=0.22, ymajorgrids=true, grid style={dashed,gray!35}
]
\addplot[fill=blue!45] coordinates {(Canonical,1.00) (Paraphrased,0.36) (Overall,0.68)};
\end{axis}
\end{tikzpicture}
\caption{Study 1 exact-case accuracy by query stratum.}
\label{fig:study1_accuracy}
\end{figure}
...
The hybrid restoration condition requested 500 missing fields and filled 499. All 499 filled values were entity-supported by the benchmark's fuzzy matching procedure, 142 exactly matched the sealed reference value, and no filled value was classified as a wrong-entity result. One field remained unresolved. These metrics should be interpreted separately: entity support indicates that the returned value was associated with the target organization, whereas exact match indicates agreement with the stored benchmark value. Non-exact entity-supported values may be valid updates or alternate representations. The result therefore supports Pathways as an assistive restoration workflow, not as an autonomous source of truth.

Table~\ref{tab:study4_examples} gives concrete examples from the benchmark. In the first example, the input record is missing only a postal code. The hybrid system returns ``M4L 2A4'', which agrees exactly with the sealed reference and is supported by the matching pharmacy name and address. In the second example, the requested website differs from the stored reference: the reference contains a directory URL, while the ARS fallback returns the pharmacy's current organizational website. The value is entity-supported but not exact, illustrating why exact match and entity support are reported separately. In the third example, the system returns no phone number; this is unresolved rather than incorrect. Finally, the Places baseline returns an address that is plausible for the named pharmacy but does not match the benchmark's expected unit-level representation. The evaluator does not automatically accept such a value without sufficient identity evidence. This conservative treatment is preferable to silently attaching a plausible value to the wrong or insufficiently supported entity.
...
\begin{table}[H]
\caption{Study 4 missing-data restoration conditions}
\label{tab:study4_results}
\centering
\small
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Condition} & \textbf{Filled} & \textbf{Supported} & \textbf{Exact} & \textbf{Wrong entity} & \textbf{Unresolved} & \textbf{Fill rate} \\
\midrule
First-result Places & 427 & 158 & 76 & 269 & 73 & 85.4\% \\
Fuzzy Places v2 & 340 & 340 & 84 & 0 & 160 & 68.0\% \\
Hybrid Places + ARS & 499 & 499 & 142 & 0 & 1 & 99.8\% \\
\bottomrule
\end{tabular}
\end{table}
...
The four Pathways evaluations provide a bounded account of the platform. Study 1 achieved micro-F1 of 0.917 but only 0.680 exact-case accuracy on a mixed canonical/paraphrase set. Study 2 found that BM25 exceeded Pathways on adjudicated nDCG@5 (0.916 versus 0.804). Study 3 showed a monotonic simulated relationship between verification participation and day-90 freshness. Study 4 filled 499 of 500 requested fields in the hybrid condition, with 499 entity-supported values, 142 exact matches, no wrong-entity fills, and one unresolved field. These results support Pathways as an auditable assistive prototype, not as a finished clinical directory or superior general-purpose search engine.
...
\begin{itemize}
    ...
    \item \textbf{Ranking and language limitations:} The current Pathways ranker did not exceed BM25 on adjudicated nDCG@5, and the extraction benchmark showed a substantial exact-case drop on paraphrased requests. These results indicate that the current fit--freshness weighting and rule-based vocabulary require further calibration and broader language coverage.
    \item \textbf{Missing-Data Restoration:} The targeted benchmark's entity-supported status was based on fuzzy matching rather than complete authoritative review of every returned value. Although the hybrid condition produced 499 supported fills and no wrong-entity classifications, only 142 matched the sealed reference exactly. These findings support practical usefulness but require source-backed validation and human review before clinical deployment.
\end{itemize}
